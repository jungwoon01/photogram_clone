...
function getSubscribeModalItem(u) {
	...
	<div class="subscribe__item" id="subscribeModalItem-${u.userId}">
		<div class="subscribe__img">
			<img src="/uplad/${u.profileImageUrl}" onerror="this.src='/images/person.jpeg'"/>
		</div>
	...
	if(!u.equalUserState){ // 동일 유저가 아닐 때 버튼이 만들어 져야한다.
		if(u.subscribeState) { // 구독중이면 구독 취소
...
function profileImageUpload(principalId) {
	$("#userProfileImageInput").click(); // 파일 input 실행(id = userProfileImageInput)
	...
	$("#userProfileImageInput").on("change", (e) => {
		...
		// FormDta 객체를 이용하면 form 태그의 필드와 그 값을 나타내는 일련의 key/value 쌍을 담을 수 있다.
		...
		$.ajax({
			...
			processData: false, // 필수 : contentType 을 false 로 줬을 때 QueryString 자동 설정되는데 그것을 해제
